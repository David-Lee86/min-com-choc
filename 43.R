...
R_value <- grobTree(textGrob(label = ~italic(R)^{"2"}~{"="}~0.26006, x=0.02, y=0.95, hjust=0, gp=gpar(col="black", fontsize=15)))
P_value <- grobTree(textGrob(label = ~italic(p)~{"<"}~"0.0001", x=0.02, y=0.87, hjust=0, gp=gpar(col="black", fontsize=15)))
...
A1 <- distBC_CAP.plot + 
  ...
  theme(axis.text.y = element_text(angle=0, hjust=0, vjust=0.5, size=15, color = "black"),
...
## Create r and p labels
R_value <- grobTree(textGrob(label = ~italic(R)^{"2"}~{"="}~0.09946, x=0.02, y=0.95, hjust=0, gp=gpar(col="black", fontsize=15)))
P_value <- grobTree(textGrob(label = ~italic(p)~{"<"}~"0.0002", x=0.02, y=0.87, hjust=0, gp=gpar(col="black", fontsize=15)))
...
## Create r and p labels
R_value <- grobTree(textGrob(label = ~italic(R)^{"2"}~{"="}~0.06103, x=0.02, y=0.95, hjust=0, gp=gpar(col="black", fontsize=15)))
P_value <- grobTree(textGrob(label = ~italic(p)~{"<"}~"0.0004", x=0.02, y=0.87, hjust=0, gp=gpar(col="black", fontsize=15)))
...
# Plot figure
Plot2 <- egg::ggarrange(A1, A2, A3, ncol = 3, widths = c(1, 1, 1))
Plot2

# Save the plot
ggsave("Fig2d.pdf", height = 12, width = 27, units = 'cm', plot = Plot2)
