...
# -------------------------------------------------------------------------------------------------
#import tree
tree <- read.tree("santander_tree-fungi_Species_v2.nw")
tree
...
p = ggtree(tree, color="black", size=0.2)  %<+% metadata +
  geom_tiplab(aes(label=taxID2), offset=1, hjust = 0, vjust=0.5, size=0, align = TRUE, linetype = "dotted", linesize = 0.2) +
  geom_tippoint(aes(fill=Phylum), size =1, alpha=1, shape=21, stroke=0) +
  scale_fill_manual(values = mypal) + guides(color=FALSE)
p + theme(legend.title = element_text(size=5, face="bold"),
...
Plot2
ggsave("Supplementary_Fig1d.pdf", height=8, width=5, units='cm', plot = Plot2)
...
A3
ggsave("Supplementary_Fig1d_legend.pdf", height=5, width=6.5, units='in', plot = A3)
