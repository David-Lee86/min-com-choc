...
final_plot <- distBC_CAP.plot + 
  ...
  annotate("text", size=4.5, label = ~italic(R)^{"2"}~{"="}~0.51774, x = 0.5, y = 1.8) +
  theme(legend.position="bottom") +
  guides(fill=guide_legend(nrow=, override.aes = list(size = 6)))
final_plot

# Save Plot
ggsave("Fig1d_ii.pdf", plot = final_plot, height = 3.8, width = 3.8, units = "in")
