...
# -------------------------------------------------------------------------------------------------
### Counting taxa with enrichment or depletion
...
tree <- drop.tip(tree, c("2741499", "2703885", "2487072",
                         "1526571", "76758", "2269374"))
tree
setdiff(tree$tip.label, metadata$taxID)

# Remove taxa from yaxis_order
yaxis_order <- setdiff(yaxis_order, c("2741499", "2703885", "2487072",
                                      "1526571", "76758", "2269374"))

#remove taxa from metadata file
metadata <- metadata %>% filter(!taxID %in% c("2487071", "1759437", "741091", "1561003"))
setdiff(metadata$taxID, tree$tip.label)

#remove taxa from tax_ID file
tax_ID <- tax_ID %>% filter(!taxID %in% c("2487071", "1759437", "741091", "1561003"))
...
# Reorder the 'Species' column based on taxID order and yaxis_order
Res_fer <- Res_fer %>%
...
A1
A1 = A1 + theme(legend.title = element_text(size=6, face="bold"),
...
Plot2
ggsave("Supplementary_Fig1a.pdf", height=17.5, width=4.5, units='cm', plot = Plot2)
...
A3
ggsave("Supplementary_Fig1a_legend.pdf", height=5, width=6.5, units='in', plot = A3)
