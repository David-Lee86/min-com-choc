...
# -------------------------------------------------------------------------------------------------
#import tree
tree <- read.tree("santander_tree-fungi_family_v2.nw")
tree
...
mypal=c('#fb4f29','#beaa2f',"#6aceee")
p = ggtree(tree, color="black", size=0.3)  %<+% metadata +
  geom_tiplab(aes(label=taxID2), offset=1, hjust = 0, vjust=0.5, size=0, align = TRUE, linetype = "dotted", linesize = 0.2) +
  geom_tippoint(aes(fill=Phylum), size =1.5 , alpha=1, shape=21, stroke=0) +
...
Plot2
ggsave("Supplementary_Fig1f.pdf", height=2.6, width=4.35, units='cm', plot = Plot2)
...
A3
ggsave("Supplementary_Fig1f_legend.pdf", height=5, width=6.5, units='in', plot = A3)
