...
carbom.sub.prun = prune_samples(sample_sums(carbom)>=1000, carbom)
...
# -------------------------------------------------------------------------------------------------
#Plot Variance
LC = read.csv("PERMANOVA_SYNCOM_16S.csv")
data_melt <- melt(LC)
...
mypal = c('white','#c4b496','#5b7a97','#003f5e', '#ede1e1')
...
#Effect of Treatment
R_value <- grobTree(textGrob(label = ~italic(R)^{"2"}~{"="}~0.41137, x=0.05, y=0.95, hjust=0, gp=gpar(col="black", fontsize=10)))
P_value <- grobTree(textGrob(label = ~italic(p)~{"<"}~"0.0001", x=0.05, y=0.87, hjust=0, gp=gpar(col="black", fontsize=10)))
...
##..................................................................................................................
#Effect of Time
R_value <- grobTree(textGrob(label = ~italic(R)^{"2"}~{"="}~0.15934, x=0.1, y=0.18, hjust=0, gp=gpar(col="black", fontsize=10)))
...
ggsave("Fig5c.pdf", height=2.7, width=6, units='in', plot = Plot5)
...
ggsave("Fig5c_legend.pdf", height=3.5, width=8.5, units='in', plot = legend)
...
ps1@sam_data$Treatment<- factor(ps1@sam_data$Treatment, levels = c("NMC","MC"))
ordcap = ordinate(ps1, "CAP", "bray", ~ Treatment + Condition(Time + Rep))
distBC_CAP.plot = plot_ordination(ps1, ordcap, "Sample_name", color="Treatment", shape="Time")
distBC_CAP.plot$layers
distBC_CAP.plot$layers <- distBC_CAP.plot$layers[-1]
mypal = c('#7e9103', '#c9de57')
A2 = distBC_CAP.plot + 
  geom_hline(yintercept = 0, colour="gray70", linetype="dashed") +
  geom_vline(xintercept = 0, colour="gray70", linetype="dashed") +
  geom_point(aes(fill = Treatment, shape = Time), size = 6, stroke = 0.6, colour = "black") +
  
  scale_fill_manual(values = mypal, labels = B, name = "Treatment") +
  scale_shape_manual(values = c("0hr" = 21, "48hr" = 22, "96hr" = 24), name = "Time") +
  
  theme_classic() +
  theme(
    axis.text.y   = element_text(angle=0, hjust = 0, vjust = 0.5, size=12, color = "black"),
    axis.text.x   = element_text(angle=0, hjust = 0.5, vjust = 1, size=12, color = "black"),
    axis.title.y  = element_text(size=12, face="bold"),
    axis.title.x  = element_text(size=12, face="bold"),
  ...
  guides(
    fill = guide_legend(nrow=1, override.aes = list(shape = 21, size = 6)), 
    shape = guide_legend(nrow=1, override.aes = list(size = 6, fill = "white"))
  ) +
  
  scale_x_continuous(expand = c(0.1, 0.1)) +
  scale_y_continuous(expand = c(0.1, 0.1)) +
  annotation_custom(R_value) + 
  annotation_custom(P_value)
A2
