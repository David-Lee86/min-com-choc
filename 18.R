...
# -------------------------------------------------------------------------------------------------
# Import data
heatmap.dataset2 = read.csv("Colombia_RF_tem_pH.csv")

# -------------------------------------------------------------------------------------------------
# Set up row names to X
row.names(heatmap.dataset2)=heatmap.dataset2$Feature

class(heatmap.dataset2)

##Convert NA values to zero
heatmap.dataset2[is.na(heatmap.dataset2)] <- 0
class(heatmap.dataset2)
...
colnames(Res_fer) <- c("Id", "variable","value")#change column names
...
p = ggplot(data=Res_fer) +
  ...
  scale_y_discrete(position = "left", label = C, expand = c(0,0)) + scale_x_discrete(label = B, expand = c(0,0)) #+ #label = B,
p
A2 = p + guides(color="none", shape="none") +
  theme(axis.text.x=element_text(angle=90, hjust = 1, vjust = 0.5, size=6, color="black", margin=margin(-1,0,0,0)),
        axis.text.y=element_text(angle=0, hjust = 1, vjust = 0.5, size=6, color="black", margin=margin(0,-1,0,0)),
  ...
  guides(fill = guide_colourbar(barwidth = 3.5,
                                barheight = 0.5,
  ...
  theme(legend.margin=margin(c(0,-5,0,-10)),
        legend.title = element_text(size = 5, angle = 0, hjust = 0.5, face="bold"),
...
Plot2 = A2a %>%
  ...
  insert_top(A5a, height=0.05) %>%
  insert_top(A6a, height=0.05) %>%
  insert_top(A3a, height=0.06) %>%
  insert_top(E, height=0.03) %>%
  insert_left(E, width=0.2)
Plot2
ggsave("Fig3c.pdf", height=1.8, width=4.5, units='in', plot = Plot2)
...
ggsave("Fig3c_legend.pdf", height=4, width=8, units='in', plot = legend)
