...
# -------------------------------------------------------------------------------------------------
# Data filtering
carbom.sub.prun <- prune_samples(sample_sums(carbom.sub) >= 500, carbom.sub)
carbom.sub.prun <- prune_taxa(taxa_sums(carbom.sub.prun) > 2, carbom.sub.prun)

# -------------------------------------------------------------------------------------------------
#subset Santander
carbom.sub.prun=subset_samples(carbom.sub.prun, Location %in% c("Santander"))

#select a specific taxa
carbom.sub.prun <- subset_taxa(carbom.sub.prun, Domain =="Bacteria")

# -------------------------------------------------------------------------------------------------
carbom2.prun.sub = carbom.sub.prun

#Merge at the species level
carbom2.prun.sub <- tax_glom(carbom2.prun.sub, "Species")
carbom2.prun.sub

#Filter method 1: remove taxa with uncharacterized (NA) Species
carbom2.prun.sub = subset_taxa(carbom2.prun.sub, !is.na(Species) & !Species %in% c("", "uncharacterized"))
carbom2.prun.sub
...
# -------------------------------------------------------------------------------------------------
### counting enrichment and delpeation numbers
# subset to only rows with Id
summary_data <- subset(Res_fer, Contrast == "res1")
# Subset the data to select rows where padj < 0.05 and log2FoldChange > 2
enriched <- subset(summary_data, padj < 0.05 & log2FoldChange > 2)
# Count the number of rows in the subsetted data
nrow(enriched)
# Subset the data to select rows where padj < 0.05 and log2FoldChange < 2
depleated <- subset(summary_data, padj < 0.05 & log2FoldChange < 2)
# Count the number of rows in the subsetted data
nrow(depleated)
...
tax_ID=tax_ID[tax_ID$ASV %in% sig_genes$Id,]
...
# -------------------------------------------------------------------------------------------------
#import tree
tree <- read.tree("santander_tree_Bacteria_species.nw")
tree
...
metadata$taxID2 = metadata$taxID#add colunm for naming
...
#taxa present in in the tree but not metadata file
setdiff(tree$tip.label, metadata$taxID)

#taxa present in metadata file bit not in the tree
setdiff(metadata$taxID, tree$tip.label)


#########################################################################################################
##edit tree, metadata and abundance file
# Remove taxa from the tree
tree <- drop.tip(tree, c("2741499", "2703885", "2487072"))
tree
setdiff(tree$tip.label, metadata$taxID)

# Remove taxa from gene_order
gene_order <- setdiff(gene_order, c("2741499", "2703885", "2487072"))

#remove taxa from metadata file
metadata <- metadata %>% filter(!taxID %in% c("2487071", "1759437", "741091"))
setdiff(metadata$taxID, tree$tip.label)

#remove taxa from tax_ID file
tax_ID <- tax_ID %>% filter(!taxID %in% c("2487071", "1759437", "741091"))
setdiff(tax_ID$taxID, tree$tip.label)
...
p = ggtree(tree, color="black", size=0.2)  %<+% metadata +
  geom_tiplab(aes(label=taxID2), offset=1, hjust = 0, vjust=0.5, size=0, align = TRUE, linetype = "dotted", linesize = 0.02) +
...
Res_fer$taxID <- factor(Res_fer$taxID, levels = gene_order)


##Reorder y axis Species by phylogenetic tree taxa order
# Reorder the 'Species' column based on taxID order and gene_order
Res_fer <- Res_fer %>%
  arrange(factor(taxID, levels = gene_order)) %>%
...
#sort heat map matrix based on the gene clustering (gene_order)
Res_fer = Res_fer %>% arrange(factor(taxID, levels = gene_order))
...
frames = select(Res_fer, c('Id','taxID','Species','log2FoldChange','padj','x_position','y_position'))
...
#Rescale fold change value
Res_fer$log2FoldChange.rescaled <- rescale(Res_fer$log2FoldChange, to = c(-2.1, 2.1))

p = ggplot(data=Res_fer) +
  geom_raster(aes(x=Contrast, y=Species, fill=log2FoldChange)) +
...
p + guides(color=FALSE, shape=FALSE, fill=FALSE) +
  theme(axis.text.x=element_text(angle=35, hjust = 1, vjust = 1, size=5, color="black"),
        axis.text.y=element_text(angle=0, hjust = 1, vjust = 0.5, size=2.1, color="black"),
...
##############################################################################################################################
#################################################################################################################################
## merge tree with heatmap 
##build phylo tree with legend

mypal=c('Acidobacteria'="black",
        'Actinobacteria'='#a0ae00',
        'Bacteroidetes'='#e0e004',
        'Fusobacteria'='#535121',
        'Proteobacteria'='#195850',
        'Synergistetes'='#e0aa02',
        'Cyanobacteria'='#f35828',
        'Firmicutes'='#de5275',
        'Tenericutes'='#8e67a0',
        'Thermodesulfobacteria'="#c1204c",
        'Thermotogae'='#5e3a49',
        'Candidatus Bipolaricaulota'='#d4c29e',
        'Spirochaetes'="pink",
        "Deinococcus-Thermus"="#6aceee")

p = ggtree(tree, color="black", size=0.2)  %<+% metadata +
  geom_tiplab(aes(label=taxID2), offset=1, hjust = 0, vjust=0.5, size=0, align = TRUE, linetype = "dotted", linesize = 0.02) +
  geom_tippoint(aes(fill=Phylum), size =0.7, alpha=1, shape=21, stroke=0) +
  scale_fill_manual(values = mypal) + guides(color=FALSE)
A1_L = p + theme(legend.title = element_text(size=6, face="bold"),
...
A1 = ggtree(tree, color="black", size=0.1)  %<+% metadata +
  ...
  geom_tippoint(aes(color=Phylum), size =0.4, alpha=1, shape=16, stroke=0) +
  scale_color_manual(values = mypal) + guides(color=FALSE, fill=FALSE) + theme(plot.margin=margin(0, 0, 0, 0))
A1


##build heatmap with legend
B=c('0h vs 24h','0h vs 48h','0h vs 72h','0h vs 96h')
p = ggplot(data=Res_fer) +
  geom_raster(aes(x=Contrast, y=Species, fill=log2FoldChange)) +
  scale_fill_gradient2(low="blue", high="red", na.value="grey", name="log2FoldChange") +
  ...
  geom_rect(data=sig_values, size=0.2, fill=NA, colour="black", aes(xmin=x_position - 0.4, xmax=x_position + 0.4, ymin=y_position - 0.4, ymax=y_position + 0.4))
p
A2_L = p + guides(color=FALSE, shape=FALSE) +
  theme(axis.text.x=element_text(angle=35, hjust = 1, vjust = 1, size=4, color="black"),
        axis.text.y=element_text(angle=0, hjust = 1, vjust = 0.5, size=1.5, color="black"),
        panel.background = element_blank(),
        axis.ticks = element_blank(),
        plot.title = element_blank(),
        axis.title.x  = element_blank(),
...
library(egg)
# Extract the legend. Returns a gtable
A2_leg <- get_legend(A2_L)
# Convert extracted legend to a ggplot
LEG2 = as_ggplot(A2_leg)


##build heatmap without legend
A2 = ggplot(data=Res_fer) +
  geom_raster(aes(x=Contrast, y=Species, fill=log2FoldChange)) +
  scale_fill_gradient2(low="blue", high="red", na.value="grey", name="log2FoldChange") +
  theme(panel.border = element_rect(colour = "black", fill=NA, size=0.4)) +
  scale_y_discrete(position = "right", expand = c(0,0)) + scale_x_discrete(label = B, expand = c(0,0)) +
  geom_rect(data=sig_values, size=0.05, fill=NA, colour="black", aes(xmin=x_position - 0.45, xmax=x_position + 0.45, ymin=y_position - 0.4, ymax=y_position + 0.4)) +
  guides(color=FALSE, shape=FALSE, fill=FALSE) +
  theme(axis.text.x=element_text(angle=90, hjust = 1, vjust = 0.5, size=4, color="black"),
        axis.text.y=element_text(angle=0, hjust = 1, vjust = 0.5, size=0.5, color="black"),
        panel.background = element_blank(),
        axis.ticks = element_blank(),
        plot.title = element_blank(),
        axis.title.x  = element_blank(),
...
##plot figure
Plot2 = egg::ggarrange(A4,A1,A2, ncol = 3, widths = c(0.4,0.3,2))
Plot2
ggsave("Extended_Data_Fig2a.pdf", height=17.5, width=4.5, units='cm', plot = Plot2)



##plot legends
A3 = egg::ggarrange(LEG, LEG2, ncol=1, heights=c(1,1))
A3
ggsave("legend.pdf", height=5, width=6.5, units='in', plot = A3)
