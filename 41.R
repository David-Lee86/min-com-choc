...
# -------------------------------------------------------------------------------------------------
#import tree
tree <- read.tree("santander_tree_Bacteria_genus_v2.nw")
tree
...
A1 = ggtree(tree, color="black", size=0.1)  %<+% metadata +
  ...
  geom_tippoint(aes(color=Phylum), size =1.2, alpha=1, shape=16, stroke=0) +
...
A2 = ggplot(data=Res_fer) +
  ...
  theme(axis.text.x=element_text(angle=90, hjust = 1, vjust = 0.5, size=4, color="black"),
        axis.text.y=element_text(angle=0, hjust = 1, vjust = 0.5, size=2.5, color="black"),
        panel.background = element_blank(),
        axis.ticks = element_blank(),
        plot.title = element_blank(),
        axis.title.x  = element_blank(),
...
Plot2
ggsave("Supplementary_Fig1b.pdf", height=17.5, width=4.5, units='cm', plot = Plot2)
...
A3
ggsave("Supplementary_Fig1b_legend.pdf", height=5, width=6.5, units='in', plot = A3)
