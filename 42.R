...
carbom2.prun.sub <- subset_taxa(carbom2.prun.sub, Domain =="Bacteria")

# -------------------------------------------------------------------------------------------------
#Merge at the Family level
carbom2.prun.sub <- tax_glom(carbom2.prun.sub, "Family")
carbom2.prun.sub

#Filter method 1: remove taxa with uncharacterized (NA) Family
carbom2.prun.sub = subset_taxa(carbom2.prun.sub, !is.na(Family) & !Family %in% c("", "uncharacterized"))
carbom2.prun.sub
...
# -------------------------------------------------------------------------------------------------
#import tree
tree <- read.tree("santander_tree-Bacteria_family_v2.nw")
tree
...
tree <- drop.tip(tree, c("2219224"))
tree
setdiff(tree$tip.label, metadata$taxID)

# Remove taxa from yaxis_order
yaxis_order <- setdiff(yaxis_order, c("2219224"))


#remove taxa from metadata file
metadata <- metadata %>% filter(!taxID %in% c("2656914"))
setdiff(metadata$taxID, tree$tip.label)

#remove taxa from tax_ID file
tax_ID <- tax_ID %>% filter(!taxID %in% c("2656914"))
...
p + theme(legend.title = element_text(size=10, face="bold"),
...
# Reorder the 'Family' column based on taxID order and yaxis_order
Res_fer <- Res_fer %>%
...
#select columns
frames = select(Res_fer, c('Id','taxID','Family','Genus','Species','log2FoldChange','padj','x_position','y_position'))
...
p = ggplot(data=Res_fer) +
  geom_raster(aes(x=Contrast, y=Family, fill=log2FoldChange)) +
...
p + guides(color=FALSE, shape=FALSE, fill=FALSE) +
  theme(axis.text.x=element_text(angle=60, hjust = 1, vjust = 1, size=6, color="black"),
        axis.text.y=element_text(angle=0, hjust = 1, vjust = 0.5, size=3, color="black"),
...
A1 = ggtree(tree, color="black", size=0.1)  %<+% metadata +
  ...
  geom_tippoint(aes(color=Phylum), size =1.5, alpha=1, shape=16, stroke=0) +
...
Plot2
ggsave("Supplementary_Fig1c.pdf", height=13, width=5, units='cm', plot = Plot2)
...
A3
ggsave("Supplementary_Fig1c_legend.pdf", height=5, width=6.5, units='in', plot = A3)
