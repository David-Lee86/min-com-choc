...
B = c("0hr"="0h",
      ...
      "72hr"="72h",
      "96hr"="96h")
R_value <- grobTree(textGrob(label = ~italic(R)^{"2"}~{"="}~0.49173, x=0.55, y=0.95, hjust=0, gp=gpar(col="black", fontsize=15)))
P_value <- grobTree(textGrob(label = ~italic(p)~{"<"}~"0.0001", x=0.55, y=0.87, hjust=0, gp=gpar(col="black", fontsize=15)))
...
R_value <- grobTree(textGrob(label = ~italic(R)^{"2"}~{"="}~0.06982, x=0.55, y=0.95, hjust=0, gp=gpar(col="black", fontsize=15)))
P_value <- grobTree(textGrob(label = ~italic(p)~{"="}~"0.0053", x=0.55, y=0.87, hjust=0, gp=gpar(col="black", fontsize=15)))
...
# Effect of Harvest
mypal <- c("plum4", "darkmagenta", "violetred")
R_value <- grobTree(textGrob(label = ~italic(R)^{"2"}~{"="}~0.03075, x=0.55, y=0.95, hjust=0, gp=gpar(col="black", fontsize=15)))
P_value <- grobTree(textGrob(label = ~italic(p)~{"="}~"0.0309", x=0.55, y=0.87, hjust=0, gp=gpar(col="black", fontsize=15)))
...
Plot2 <- ggarrange(A1, A2, A3, ncol = 3, widths = c(1,1,1))
ggsave("Fig2c.pdf", height = 12, width = 27, units = 'cm', plot = Plot2)
